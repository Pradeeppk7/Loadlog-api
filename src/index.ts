...
<html lang="en">
  ...
  users {
        ...
        <section class="coach-card">
          ...
          <div class="coach-form">
            <div class="coach-field">
              <label for="coachGoal">Goal</label>
              <textarea id="coachGoal" placeholder="Example: lose fat while keeping strength"></textarea>
            </div>
            <div class="coach-field">
              <label for="coachDiet">Dietary Preferences</label>
              <textarea id="coachDiet" placeholder="Example: vegetarian, high protein, lactose free"></textarea>
            </div>
            <div class="coach-field">
              <label for="coachLimits">Injuries or Limitations</label>
              <textarea id="coachLimits" placeholder="Example: shoulder pain during overhead pressing"></textarea>
            </div>
            <div class="quick-prompts">
              <button type="button" data-coach-prompt="Make me a simple 3-day beginner workout split with progressive overload advice.">3-Day Split</button>
              <button type="button" data-coach-prompt="How much protein should I eat each day if I want to build muscle?">Protein Help</button>
              <button type="button" data-coach-prompt="My bench press has stalled. What should I change in training and recovery?">Bench Plateau</button>
              <button type="button" data-coach-prompt="Give me a one-day high-protein vegetarian meal plan.">Meal Plan</button>
            </div>
          </div>
        </section>
        <section class="coach-card coach-chat">
          <div class="panel-header">
            <strong>Coach Chat</strong>
            <span class="coach-status" id="coachStatus">Ready</span>
          </div>
          <div class="coach-messages" id="coachMessages">
            <div class="coach-message assistant">I’m your LoadLog Coach. Ask a training, recovery, or nutrition question.</div>
          </div>
          <div class="coach-composer">
            <textarea id="coachMessageInput" placeholder="Ask about workouts, meals, recovery, or nutrition..."></textarea>
            <div class="coach-actions">
              <button id="coachSendButton" type="button">Send</button>
              <button id="coachClearButton" type="button" class="secondary">Clear Chat</button>
            </div>
          </div>
        </section>
      </div>
    ...
    <script>
      ...
      const coachSendButton = document.getElementById("coachSendButton");
      const coachClearButton = document.getElementById("coachClearButton");
      const coachHistory = [];
      ...
      function appendCoachMessage(role, content) {
        ...
        coachMessages.appendChild(item);
        coachMessages.scrollTop = coachMessages.scrollHeight;
      }

      function getCoachProfile() {
        return {
          goal: document.getElementById("coachGoal").value.trim(),
          dietaryPreferences: document.getElementById("coachDiet").value.trim(),
          injuriesOrLimitations: document.getElementById("coachLimits").value.trim(),
        };
      }
      ...
      async function sendCoachMessage() {
        ...
        try {
          ...
          appendCoachMessage("assistant", payload.reply);
          coachHistory.push({ role: "assistant", content: payload.reply });
          setCoachStatus("Ready");
        } catch (error) {
          const messageText = error instanceof Error ? error.message : String(error);
          appendCoachMessage("assistant", "Error: " + messageText);
          coachHistory.push({ role: "assistant", content: "Error: " + messageText });
          setCoachStatus("Failed");
        }
      }

      coachSendButton.addEventListener("click", () => {
        sendCoachMessage();
      });

      coachMessageInput.addEventListener("keydown", (event) => {
      ...
      coachClearButton.addEventListener("click", () => {
        coachHistory.length = 0;
        coachMessages.innerHTML = "";
        appendCoachMessage("assistant", "I’m your LoadLog Coach. Ask a training, recovery, or nutrition question.");
        setCoachStatus("Ready");
      });

      document.querySelectorAll("[data-coach-prompt]").forEach((button) => {
        button.addEventListener("click", () => {
          coachMessageInput.value = button.getAttribute("data-coach-prompt");
          coachMessageInput.focus();
        });
      });
    </script>
  </body>
